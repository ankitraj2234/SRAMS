...
ipcMain.handle('login', async (event, email, password) => {
    try {
        ...
        // Store fingerprint for browser session sync
        currentFingerprint = certResult.fingerprint;
...
ipcMain.handle('open-dashboard', async () => {
    ...
    // Also pass fingerprint to align browser session with desktop session
    const url = `${frontendUrl}/login?auto=1&token=${encodeURIComponent(accessToken)}&desktop_session=${encodeURIComponent(desktopSessionToken)}&refresh=${encodeURIComponent(refreshToken || '')}&fingerprint=${encodeURIComponent(currentFingerprint || '')}`;
